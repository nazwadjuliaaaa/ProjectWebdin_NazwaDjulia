// Pertemuan 15: CRUD Daftar User dan Reset Password Page
"use client";
...
export default function UsersPage() {
  ...
  const handleResetPassword = async (id: number, nama: string) => {
    if (!window.confirm(`Yakin ingin mereset password untuk user "${nama}"?`)) return;

    try {
      setMessage("");
      setError("");
      const tempPassword = await resetPasswordApi(id);
      setMessage(`Password berhasil direset. Password sementara untuk "${nama}": ${tempPassword}`);
    } catch (err) {
